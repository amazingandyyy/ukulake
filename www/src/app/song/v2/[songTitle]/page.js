...
export default function Page ({ params }) {
  ...
  return (<div className='flex flex-col h-screen'>
      <div className="flex bg-white shadow-sm col-span-3 h-16 p-4 flex-row items-center justify-between">
        <a href='/'>
          <div className='self-start font-bold text-teal-700 text-2xl'><span className='bg-teal-700 text-white rounded-lg'>˙ᵕ˙</span><span className='pl-1 text-gray-900'>Ukulake</span></div>
        </a>
        <div className='flex'>
          ...
          <div className='rounded-xl p-2 bg-gray-100 m-1 hover:bg-teal-600 hover:text-white cursor-pointer'>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
    ...
    <div className="grid grid-cols-3 grid-rows-3 grid-container p-4">
      <div className="col-span-3 md:col-span-1 row-span-1 md:row-span-3 rounded-xl">
        <iframe src={`${videoSrc}?start=0`} width='100%' height='100%' className='rounded-xl shadow-xl' frameBorder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowFullScreen />
      </div>
      <div className="col-span-3 md:col-span-2 row-span-3 pt-4 md:pt-0 md:pl-4">
        <iframe className='rounded-xl shadow-xl' width='100%' height='100%' src={tabSrc} />
      </div>
    </div>
  </div>)
}
