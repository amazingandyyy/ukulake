      ...
      <div className='grid grid-cols-3 grow-rows-8 md:grid-rows-4 p-2 md:p-4 gap-1'>
        {
          data.map(song => {
            return (
              <a href={`/song/v3/${encodeURIComponent(song.title)}`} key={song.title} className='col-span-3 md:col-span-1 hover:opacity-70 bg-gray-50 rounded-md p-2 px-4 hover:text-teal-800'>
